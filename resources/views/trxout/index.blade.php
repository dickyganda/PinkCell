            ...
            <div class="card-body">
             <a href="" class="btn btn-success btn-xs" title="Tambah Data Baru" role="button" data-toggle="modal" data-target="#modaltambahdata"><i class="fas fa-plus-circle"></i></a><br><br>
              ...
              <table id="dt-basic-example" class="table table-bordered table-responsive table-hover table-striped">
                                                ...
                                                <tbody height="10px">
                                                    @php $i=1 @endphp
                                                @foreach($trxout as $out)
                                                    <tr>
                                                        <td>{{ $i++ }}</td>
                                                        <td>{{ $carbon::parse($in->TglKeluar)->format('d/m/Y H:i:s') }}</td>
                                                        <td>{{ $out->ImeiKeluar}}</td>
                                                        <td>{{ $out->MerkKeluar }}</td>
                                                        <td>{{ $out->TypeKeluar }}</td>
                                                        <td>{{ $out->RamKeluar.'/'.$out->RomKeluar }}</td>
                                                        <td>@currency( $out->HargaKeluar )</td>
                                                        <td></td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
